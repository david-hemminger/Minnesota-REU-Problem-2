...
We further show that cover transitivity is preserved under semidirect products, allowing us to describe several large families of CCT actions in Section \ref{ssec:CCT_examples}.
...
Denote the category of finite graded posets by $\mathcal{P}$, and let $P,Q$ be finite graded posets.  Throughout the paper we write $x\le_P y$ to denote that $x$ is less than or equal to $y$ under the relation defined on the poset $P$.  When the poset is clear we omit the $P$ and simply write $x\le y$.  
...
We say that a group $G$ acts on $P$ if the action defines an embedding $G\hookrightarrow \Aut(P)$, and define the \textit{quotient poset} $P/G$ to be the poset whose elements are the orbits of $G$, with the relation $\mathcal{O}\le \mathcal{O}^\prime$ if there exist $x\in \mathcal{O}$, $x^\prime\in \mathcal{O}^\prime$ such that $x\le_{P} x^\prime$.  We will use the following result later in the paper.
...
\ref{lem:commuting_op_e}, the composition $F\circ \mathcal E(f)\colon\mathcal E(P) \rightarrow \mathcal E(P)^{\op}$ defines an isomorphism, so $\mathcal E(P)$ is self-dual.
\end{proof}
...
\begin{note}
For $x \in P$, denote $D(x) = \{y \in P\colon y \leq x\}$.
\end{note}
\begin{prop}
...
\begin{enumerate}
  \item For any $n \in \mathbb N$, and $G$ an elementary abelian 2-group, every $G$-action $\phi$ on $B_n$ is CCT.
...
Let $n,k$ be arbitrary. Let $G = \mathbb Z_2^k$ and pick any group action $\phi\colon G\times [n] \rightarrow [n]$. Then the induced action $\phi\colon G \times B_n \rightarrow B_n$ is CCT.
\end{lem}
...
\begin{prop}
\label{prop:regular_action_CCT}
Let $G$ be a finite group, acting on itself by left multiplication, $\psi\colon G\times G \rightarrow G,(g,h)\mapsto g\cdot h$, with $|G| = n$. By identifying $G \cong [n]$ as sets, $\psi$ this induces an action $\phi\colon G\times B_n \rightarrow B_n$. Then, $\phi$ is CCT if and only if $G \cong \mathbb Z_2^k$ for some $k$.
\end{prop}
\begin{proof}
...
\begin{rem}
There are several other results related to $\mathcal E(C_n),\mathcal E(D_n)$, with $C_n = \mathbb Z/n\mathbb Z$, which are proven in \cite[Section 7]{reu_report}. Notably, we showed 
\begin{enumerate}
	...
	\item For all $n$, $\mathcal E(B_n/D_n)$ is symmetric and unimodal.
\end{enumerate}
\end{rem}
...
A {\it partition} is a sequence of numbers $\lambda = (\lambda_1,\ldots, \lambda_k)$ such that $\lambda_1 \geq \lambda_2 \geq \cdots \geq \lambda_k$. If $\sum_{i=1}^k \lambda_i = n$ then $\lambda$ is a {\it partition of $n$}, notated $\lambda \vdash n$. Let $P_n(l,m)$ denote the set of partitions $\lambda = (\lambda_1,\ldots, \lambda_k) \vdash n$, such that $\lambda_1 \leq m,k \leq l$. That is, $P_n(l,m)$ is the partitions which fit inside an $l \times m$ rectangle.
...
\begin{rem}
Observe that Theorem \ref{thm:cover_transitive_implies_Peck} trivially implies that $\mathcal E(B_n)$ is Peck. In fact, it is true that $\mathcal E(B_n)$ is unitary Peck. For a proof, see \cite[Section 8]{reu_report}.
\end{rem}
...
As mentioned in the introduction, a natural extension of Conjecture ~\ref{conj:F_of_BnG_Peck} would be an analogous result for q-analogs. We suspect the method used in \cite[Section 8]{reu_report} to prove $\mathcal E(B_n)$ is unitary peck may solve Question ~\ref{question:unitary_peck_q_edge}.
