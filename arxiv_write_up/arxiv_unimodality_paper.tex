...
\end{conjecture}
\todo{Not a big issue here, just to make sure: is $Aut(B_n)$ a standard notation?}
...
Let $V(P)$ and $V(P_i)$ be the complex vector spaces with bases $\{x :x\in P\}$ and $\{x :x\in P_i\}$ respectively.  Note that we will frequently abuse notation and write $P$ and $P_i$ for $V(P)$ and $V(P_i)$ when our implications are clear.  In determining whether $P$ is Peck, it is often useful to consider certain linear transformations on $V(P)$.
...
\todo{Make a remark about a Generalization of $\mathcal H,\mathcal E$}
...
\todo{Make some comment about Polya theory here, or elsewhere perhaps, with reference to our full paper}
...
\todo{Why is point 2 being singled out in this remark? Doesn't Peckness apply to all three actions from proposition 1.6? Z: Did someone add this remark? I think it is weird to single out point 2 as well.}
...
\todo{refer to our other paper here perhaps. Z: I think this remark can be dropped.}
