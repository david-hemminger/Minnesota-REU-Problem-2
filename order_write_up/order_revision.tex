...
\todo{Aaron: Is this sufficient detail, or should we spell it out}
...
\end{proposition}
\begin{proof}
Note that the wreath product $G \wr S_\ell$ can be viewed as a semidirect product $G^\ell \rtimes S_\ell$. Since the action of $G$ on $P$ is CCT we obtain that the action of $G^\ell$ on $P^\ell$ is CCT by Proposition ~\ref{prop:direct_product_preservation}.  Furthermore the action $S_\ell \times (P/G)^\ell \rightarrow (P/G)^\ell$ defined by $(\sigma ,(x_1,\ldots, x_\ell)) \mapsto (x_{\sigma(1)}\ldots, x_{\sigma(\ell)})$ for $\sigma \in S_\ell$ and $x_i \in P/G$ is CCT by Lemma ~\ref{lem:symmetric_group_product_action}.  Since $P^\ell/G^\ell \cong (P/G)^\ell$, it follows that the action $\phi$ satisfies the conditions of Proposition ~\ref{prop:semidirect_product_preservation}, so $\phi$ is CCT.
\end{proof}
...
Aaron: I'm fine with moving this to the introduction, but I don't think we should call it a corollary. Should we perhaps make this a separate theorem or proposition, and just refer people to the REU report?}
\begin{remark}
...
Aaron: I think it's fine not to change anything else, just mention what we don't change in the additional document.
}
