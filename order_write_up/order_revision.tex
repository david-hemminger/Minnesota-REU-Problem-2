...
What should we do?
-Aaron
I would follow the advice of citing the REU report, but mention that it is not published to the reviewers/editor -- if they are not happy with that, we change it back. -- if we agree then this could be done in next editing - Z
Ok, I am happy to follow their advice then. I think we should make a comment in our responses about why we did this, though}
...
\begin{rmk}
	 In fact, it is true that $\mathcal E(B_n)$ is unitary Peck. For a proof, see Section 8 of the REU report. \todo{Citation edit needed for "the REU report"}
 \end{rmk}
...
Let $V(P)$ and $V(P_i)$ be the complex vector spaces with bases $\{x :x\in P\}$ and $\{x :x\in P_i\}$ respectively.  Note that we will frequently abuse notation and write $P$ and $P_i$ for $V(P)$ and $V(P_i)$ \todo{do we every actually do this?  If not, this line should be removed. --David} when what we mean is clear from context.  In determining whether $P$ is Peck, it is often useful to consider certain linear transformations on $V(P)$.
...
%%%%%%%%%%%%
%%%%%%%%%%%%
\todo{This "sub-outline" seems unnecessary for me -- is there any objection of getting rid of this? -- Zijian Aaron: I usually like these outlines, as they help me find my place in the article, and it's only three lines. But, if you feel strongly, feel free to delete it.}
...
G(x,y) &\mapsto (Gx,Gy)
\end{align*}
 is always a surjection.\todo{I think we should remove this sentence.  Since the proof of Lemma 3.15 is already pretty short, we nearly have the problem of a map as large as the territory it is mapping.}
...
Suppose $G \not \cong \mathbb Z_2^k$. Then there exists $g \in G$ such that $g^2 \neq e$. Clearly $\{e\}\lessdot \{e \cup g\},\{g\} \lessdot \{e \cup g\}$, and $\{g\} \in G\{e\}$. So in order to show that the induced action $\phi\colon G \times B_n \rightarrow B_n$ is not CCT, it suffices to show there is no $h \in G$ such that $h \in \Stab(\{e \cup g\})$ and $h\cdot \{e\} =\{g\}$.  If $h \in \Stab(\{e \cup g\})$ and $h \cdot \{e\} = \{g\}$, then $h = g$, and it follows that $g^2 = e$.  Thus there does not exist such an $h$, and the action induced by left multiplication is not CCT.\todo{check this paragraph for correctness please!}
\end{proof}

\sssec{The proof of \ref{prop:cover_transitive_building_blocks}}
\label{sssec:dihedral}\indent 

Let $x, y \in B_n$, and let $G$ act on $B_n$. \todo{As far as I can tell, I have removed every abuse of notation of this type.  I propose we remove everything from here up to Proposition 1.8. -- David}For the remainder of this section, rather then using $(Gx, Gy)$ to properly denote $\left\{ (a,b): a \in Gx, b \in Gy \right\}$,
