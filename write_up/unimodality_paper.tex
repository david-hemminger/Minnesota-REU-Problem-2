...
\begin{lem}
\label{bijection_h_f}
The map $f:\mathcal H^r(P) \rightarrow \mathcal E^r(P),G(x, y) \mapsto G(x, y)$ is a bijection.
\end{lem}
\begin{proof}
...
Next, Observe that $G \times H$ can be viewed as the semidirect product of $(G\times \{e\}) \rtimes (\{e\} \times H).$ Since $G$ acts CC transitively on $P,$ by the above paragraph, $G\times \{e\}$ acts CC transitively on $P \times Q.$ Also, since $H$ acts CC transitively on $Q,$ it follows that $\{e\}\times H$ acts CC transitively on $(P/G) \times Q.$ Therefore, the actions of $(G\times \{e\}) \rtimes (\{e\} \times H)$ satisfies the conditions of Proposition ~\ref{prop:semidirect_product_cover_transitive_actions} and so the action of $G\times H$ is CCT.
\end{proof}
...
Let $G = D_{2n}$ or $G = D_{4n}$ where $n$ is prime , and $p_i, q_i$ as defined above. Then $p_i = q_i$. In other words, the action of $D_{2n}$ or $D_{4n}$ on $B_n$ is CCT. 
\end{lem}
