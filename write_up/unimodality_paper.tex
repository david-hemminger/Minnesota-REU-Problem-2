...
Here's a dumb edit to test weird merge conflicts.
...
A group action of $G$ on $P$ is \textit{cover transitive} if whenever $x,y,z\in P$ such that $x\lessdot z$, $y\lessdot z$, and $y\in Gx$, there exists some $g\in G$ such that $g\cdot x = y$ and $g\cdot z = z$.\todo{for consiceness, would it make sense to say $g\in \Stab(z)$ with $g\cdot x =y$ throughout the paper?}
\end{defn}
...
\todo{I don't think the details of the proof need to be included here, but I've left them here commented out in case we change our minds.}
...
If $f\colon P\rightarrow Q$ is morphism of finited ordered posets, then $\mathcal{F}^r(f)\colon \mathcal{F}^r(P)\rightarrow \mathcal{F}^r(Q)$ is a morphism of finited ordered posets.
\end{lem}

\begin{proof}
$\mathcal{F}^r(f)$ is rank-preserving since for all $x\otimes y\in \mathcal{F}^r(P)$ we have $\rk_{\mathcal{F}}(x\otimes y) = \rk_P(x) = \rk_P(f(x)) = \rk_{\mathcal{F}}(\mathcal{F}^r(f)(x\otimes y))$.  Suppose $x\otimes y\le_{\mathcal{F}} x^\prime \otimes y^\prime$.  Then $x\le_P x^\prime$, $y\le_P y^\prime$, and since $f$ is order-presevering, it follows that $f(x)\le_P f(x^\prime)$, $f(y)\le_P f(y^\prime)$, hence $\mathcal{F}^r(f)(x\otimes y) \le_{\mathcal{F}} \mathcal{F}^r(f)(x^\prime \otimes y^\prime)$.  Thus $\mathcal{F}^r(f)$ is order preserving and hence a morphism of finite ordered posets.
\end{proof}
...
\begin{proof}
By Lemmas \ref{lem:f_partial_order}, \ref{lem:FP_graded_poset}, and \ref{lem:Ff_poset_morphism} we have that $\mathcal{F}^r$ takes elements in $\mathcal{P}$ to elements in $\mathcal{P}$ and morphisms of $\mathcal{P}$ to morphisms of $\mathcal{P}$.  Furthermore it is clear that $\mathcal{F}^r(\id_P) = \id_{\mathcal{F}^r(P)}$, so it remains to be shown that for finited graded posets $P, Q,$ and $R$ with morphisms $f\colon P\rightarrow Q$ and $g\colon Q\rightarrow R$ that $\mathcal{F}^r(g\circ f) = \mathcal{F}^r(g)\circ\mathcal{F}^r(f)$.  This is clear, however, as for all $x\otimes y\in \mathcal{F}^r(P)$ we have $\mathcal{F}^r(g\circ f)(x\otimes y) = g(f(x))\otimes g(f(y)) = \left(\mathcal{F}^r(g)\circ\mathcal{F}^r(f)\right)(x\otimes y)$.
\end{proof}



Given a group action of $G$ on $P$, we can easily define a natural group action of $G$ on $\mathcal{F}^r(P)$ using Proposition \ref{prop:F_well_defined}.  For all $g\in G$ we have that $g$ is an automorphism of $P$, so it follows that $\mathcal{F}^r(g)$ is an automorphism of $\mathcal{F}^r(P)$.  Since $\mathcal{F}^r$ is a functor this gives a well-defined group action.

\begin{defn}\label{defn:G_action_on_FP}
