...
In particular, let the \textit{boolean algebra of rank $n$}, denoted $B_n$, be the poset whose elements are subsets of $\{1,\ldots,n\}$ with the relation given by containment, i.e. for all $x,y\in B_n$, $x\le y$ if $x$ is a subset of $y$. For the remainder of this paper, all group actions on posets $\phi:G \times P \rightarrow P$ are assumed to be order preserving and rank preserving, meaning for all $g \in G,x \leq y \iff gx \leq gy,\rk(gx) = rk(x).$ Using Theorem \ref{thm:quotients_of_unitary_peck_posets} and the fact that $B_n$ is unitary peck (which follows from \cite[Theorem 2a]{quotients_stanley} by noting that $B_k = (B_1)^k,$) if $G$ is any action on $B_n,$ then $B_n/G$ is Peck.  We conjecture the following.
...
\end{enumerate}
\end{restatable}
\todo{restate this at the appropriate place in the dihedral group subsection}
...
\begin{rem}
For all $\vec r,\mathcal E^{\vec r}$ is a functor, for the same reasons that $\mathcal E^r$ is a functor. This generalization is essentially taking the nerves of the poset $P.$ See \cite{babson} for some related constructions, although their constructions are not completely analogous.
\end{rem}
...
\section{Rank unimodality for Non-CCT actions}
%%%%%%%%%%%%%%%%%  Cyclic  %%%%%%%%%%%%%%%%%%
...
%Let $(Gx, Gy)$  be an element in $(\mathcal E^1(B_p/G))_i$, in other words, an edge class in the necklace poset with representative $(x, y)$. Note that we view $G(x , y)$ as a subset of $\mathcal E^1 (B_p)$, namely $G(x, y) = \{(gx, g y) : g \in G \}$. Clearly $G(x , y) \subset (Gx,Gy)$ as subsets of $\mathcal E^1 (B_p)$. It is clear that the number of distinct $\sigma \in G$ such that $\sigma x \lessdot y$ is precisely the number of $G$-orbits in $(Gx, Gy)$. In particular, if the only $\sigma $ such that $\sigma x \lessdot y$ is identity, then $(Gx, Gy)=G(x, y)$ as subsets of $\mathcal E^1 (B_p)$. 
...
First we prove several sublemmas. 
