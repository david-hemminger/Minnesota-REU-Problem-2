...
\end{note}\todo{Looking back on it, I don't think we should use the notation $\mathcal E(U)^{n-2i-1}$.  For here, why don't we just say that $M^{n-2i-1}$ is the Lefschetz map?}
\begin{note}
...
\todo{There are a number of $(a\setminus b)$'s that I believe should be $(b\setminus a)$'s since $a\subset b$.}

For a particular $(x, y),(a , b)$, if either $y \subset a$ or $y \not \subset a, x\subset a, y\subset b,$ \todo{I think quantifiers are supposed to be either ($y \subset a$) or ($y \not \subset a, x\subset a, y\subset b,$), in which case an "and" needs to be added to the list of three} then $(a, b) \not >_{ \mathcal H(B_n)} (x, y),$\todo{should this be $\not >_{ \mathcal H(B_n)}$?} and so the coefficient of $(a, b)$ in $M^{n-2i-1}(x, y)$ is 0.
...
For the moment, fix $j$ and consider the set of all sequences of the form in Equation ~\eqref{eqn:f_seq} such that $(a\setminus b) \cup y_{j-1} = y_j$ First, let us show the number of such sequences with this $j$ fixed is $(k-1)!2^j.$ To do this, start by considering the number of sequences $y_0,\ldots, y_k$ such that $y=y_0 \lessdot_{B_n} y_1 \lessdot_{B_n} \cdots \lessdot_{B_n} y_k = b.$ Since we enforce $y_j = y_{j-1} \cup (a \setminus b),$ the number of such sequences is precisely the number of ways to order the elements $a \setminus y.$ Since $|y| - |a| = k-1,$\todo{$|a| - |y|$?} there are $(k-1)!$ such ways. Additionally, for $l <j$ we must have $x_{l+1} = x_l \cup (y_{l+1} \setminus y_l)$ or $x_{l+1} = x_l \cup (y_l \setminus x_l).$ 
Either of these is possible at every step \todo{I think they're only both possible for $l < j-1$.  In general this paragraph and the bit following mixes between $j$ and $j-1$ a lot.  It looks like they should all be $j-1$'s, but someone should check me on this.}. Additionally, for $l \geq j,$ we must have $x_{l+1} = x_l \cup (y_{l+1} \setminus y_l).$ So for any fixed sequence of $y_k$ with so that $j$ is minimal with $(a\setminus b) \in y_j,$ 
there are precisely $2^{j-1}$ possible sequences 
$$x=x_0 \lessdot_{B_n} x_1 \lessdot_{B_n} \cdots \lessdot_{B_n} x_k = a$$
so that 
$$(x, y) = (x_0, y_0) \lessdot_{\mathcal E(B_n)} (x_1, y_1) \lessdot_{\mathcal E(B_n)}  \cdots \lessdot_{\mathcal E(B_n)}  (x_k, y_k) = (a, b).$$

So, in total, there are $2^j(k-1)!$ sequences of the form in ~\eqref{eqn:f_seq} with $(a\setminus b) \cup y_{j-1} = y_j$. Now, there clearly must be some\todo{exactly one?} such $j,1 \leq j \leq k$\todo{"...such that this is the case."}. Therefore, the coefficient we are looking for is 
...
\end{align*}
\todo{it doesn't look like the penultimate equality holds here.  In particular I don't think the constant next to the second sum is correct, because it does not account for the extra $x\subset a$ condition on the sum that it came from.}
...
\label{w_equivalence}
We have 
\todo{looks like the right hand side of the equality, the first $x\cup \{t\}$ should be a $y$.}
\begin{align*}
  w_s = \sum_{t\notin s}^{} \left(\beta \binom {|s|-i-1}{n-2i-2}  \sum_{y \subset s}^{}(x, x \cup \{t\})  + \binom {|s|-i}{n-2i-1} \sum_{x \subset s}^{}(x ,x \cup \{t\}) \right).
\end{align*}
\end{lem}

\begin{proof}\todo{it looks like it should be $y\subset a\cup \{t\}$ and $y\not\subset a$ throughout}
\begin{align*}
...
\end{align*}
\todo{looks like last line is missing a constant by the second of the three sums}
...
\end{lem}
\todo{the $x\subset y$'s here are redundant, right?}
\begin{proof}
...
\end{align*}
\todo{I think there's an extra constant next to the second sum after the penultimate equality}
...
\end{note}\todo{$u_s$ is not defined for $|s| < n - i$ because $w_s$ is not defined for these $s$.  Please make sure that only $u_s$ for $|s| = n-i$ are needed}
...
Let $I$ be fixed, $|I| \geq 2$. If $l_J$ lies in the span of $v_{(a, b)}$ for all $J \subset I,$ then so does $l_I.$
...
The poset $\mathcal E(B_n)$ is unitary Peck.\todo{for $n > 2$?}
\end{cor}
\begin{proof}
