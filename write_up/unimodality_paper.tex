...
\todo{Put as a question whether $\mathcal E^r(B_n)$ is unitary Peck?}
\begin{defn}
\label{defn:h_map}
...
In this subsubsection, we show that any embedding of $\BBZ_2^k$ into $S_n$ defines an action on $B_n$ is CCT. In particular, this implies that the left multiplication action of $\BBZ_2^k,$ that is the action of $\BBZ_2^k \times \BBZ_2^k \rightarrow \BBZ_2^k,(g,h) \mapsto g\cdot h$ is a CCT action. However, it turns out that this is the only class of groups for which the left multiplication action is CCT.
...
\label{sec:polya}
In this section, we use the generalized notion of $\mathcal E^{\vec r}(B_n)$ to obtain the unimodality of the coefficients a certain class of polynomials from Polya Theory, which will turn out to be the rank generating function of $\mathcal E^{\vec r}(B_n)/G.$ The result we prove is a generalization of \cite[Corollary 7.16]{algebraic_stanley}. 
...
$$\mathcal E_G(r_1,\ldots, r_k) = Z_G\left(\sum_{i=1}^k r_i,\sum_{i=1}^k r_i^2,\ldots, \sum_{i=1}^k r_i^n\right).$$
\end{thm}
...
$$\left[Z_G \left(\sum_{i=1}^k r_i,\sum_{i=1}^k r_i^2,\ldots, \sum_{i=1}^k r_i^n \right)\right]_{r_1^{i_1} \cdots r_k^{i_k}}=|(\mathcal E^{i_2,\ldots, i_{k-1}}(B_n)/G)_{i_1}|.$$
\end{lem}
\begin{proof}
...
$$\left[Z_G\left(\sum_{i=1}^k r_i,\sum_{i=1}^k r_i^2,\ldots, \sum_{i=1}^k r_i^n\right)\right]_{r_1^{i_1} \cdots r_k^{i_k}}=\kappa(i_1,\ldots, i_k)=|(\mathcal E^{i_2,\ldots, i_{k-1}}(B_n)/G)_{i_1}|.$$
\end{proof}
...
In this subsection, we draw on the methods developed earlier in this section to write down some interesting generating functions for the case that $G = S_m \wr S_l.$
