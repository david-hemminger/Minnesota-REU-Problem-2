...
By Corollary \ref{cor:quotiented_edge_peck}, $\mathcal{E}(B_n)/G$ is Peck for any group action of $G$ on $B_n.$ Since the $G$-action is CCT, there is a bijective morphism from $\mathcal{E}(B_n)/G$ to $\mathcal{E}(B_n/G)$, hence $\mathcal{E}(B_n/G)$ is Peck by Lemma \ref{lem:bijection_peck_implication}.
\end{proof}
...
There is an equality $|(\mathcal E(B_n)/G)_i| = |\mathcal E(B_n/G)_i| = p_{i+1}(l,m,1).$ In particular, Theorem \ref{thm:pak_thm} holds in the case $r = 1.$
\end{prop}
\begin{proof}
...
\begin{note}
For this section, we will let $M$ be the Lefschetz map on $\mathcal E(B_n)$, as defined in ~\ref{defn:lefschetz}.
\end{note}
\begin{note}
For the remainder of this section, we fix $n$, take $i$ with $0 \leq i < n/2,$ and implicitly suppose $|a| = n-i-1,|b|= n-i,|x| = i,|y| = i+1.$ Let $k = n-2i-1,$ that is, $k = |a| - |x|.$ Additionally, whenever we write an expression of the form $(x, y)$ or $(a, b),$ it is assumed that $x \lessdot y,a \lessdot b.$ We shall also implicitly sum over all $x \subset y$ in what follows.
\end{note}
...
For a particular $(x, y),(a , b)$, if neither $y \subset a$ holds nor all three of $y \not \subset a, x\subset a,$ and $y\subset b$ hold, then $(a, b) \not >_{ \mathcal E(B_n)} (x, y),$ and so the coefficient of $(a, b)$ in $M^{n-2i-1}(x, y)$ would then be 0.
...
For the moment, fix $j$ and consider the set of all sequences of the form in Equation ~\eqref{eqn:f_seq} such that $(b\setminus a) \cup y_{j-1} = y_j$ First, let us show the number of such sequences with this $j$ fixed is $(k-1)!2^j.$ To do this, start by considering the number of sequences $y_0,\ldots, y_k$ such that $y=y_0 \lessdot_{B_n} y_1 \lessdot_{B_n} \cdots \lessdot_{B_n} y_k = b.$ Since we enforce $y_j = y_{j-1} \cup (b \setminus a),$ the number of such sequences is precisely the number of ways to order the elements of $a \setminus y.$ Since $|a| - |y| = k-1,$ there are $(k-1)!$ such ways. Additionally, for $l <j-1$ we must have $x_{l+1} = x_l \cup (y_{l+1} \setminus y_l)$ or $x_{l+1} = x_l \cup (y_l \setminus x_l).$

Either of these is possible at every step. Additionally, for $l \geq j-1,$ we must have $x_{l+1} = x_l \cup (y_{l+1} \setminus y_l).$ So for any fixed sequence of $y_k$ with so that $j$ is minimal with $(b\setminus a) \in y_j,$ 
there are precisely $2^{j-1}$ possible sequences 
$$x=x_0 \lessdot_{B_n} x_1 \lessdot_{B_n} \cdots \lessdot_{B_n} x_k = a$$
so that 
$$(x, y) = (x_0, y_0) \lessdot_{\mathcal E(B_n)} (x_1, y_1) \lessdot_{\mathcal E(B_n)}  \cdots \lessdot_{\mathcal E(B_n)}  (x_k, y_k) = (a, b).$$

So, in total, there are $2^{j-1}(k-1)!$ sequences of the form in ~\eqref{eqn:f_seq} with $(b\setminus a) \cup y_{j-1} = y_j$. Now, there clearly must be exactly one $j,1 \leq j \leq k$ such that this is the case. Therefore, the coefficient we are looking for is 
...
In this subsection, we will show the rows of $M$ form a basis by showing we can make a change of basis to a map which takes $M^{n-2i-1}$ to $L^{n-2i-1}.$ Since we know $L^{n-2i-1}$ is an isomorphism, it will follow that $M^{n-2i-1}$ is as well.
...
\begin{lem}
\label{z_equivalence}
For any set $s$ of size at least $n-i,$ we have $z_s = \sum_{y \subset s}^{}(x, y).$ In particular, $\sum_{y \subset s}^{}(x, y)$ lies in the span of the $v_{(a, b)}$
\end{lem}
\begin{proof}
We have 
\begin{align*}
  ...
  & =\left(\beta\binom {|s|-i-1}{n-2i-2}(|s|-n+i+1)+\binom{|s|-i-1}{n-2i-1}\right) \sum_{y \subset s}^{}(x,y).
\end{align*}
...
In going from the fifth line to the sixth line, for $y,s$ fixed, we count the number of $b$ with $y \subset b \subset s.$ This is exactly $\binom{|s|-i-1}{n-2i-1}$. Then, for each of these possibilities, if we additionally fix $x$, the value of $a$ such that $x \subset a, y \not \subset a$ is uniquely determined by $a = b \setminus (y \setminus x).$
\end{proof}
...
We have 
\begin{align*}
  w_s = \sum_{t\notin s}^{} \left(\beta \binom {|s|-i-1}{n-2i-2}  \sum_{y \subset s}^{}(x, y)  + \binom {|s|-i}{n-2i-1} \sum_{x \subset s}^{}(x ,x \cup \{t\}) \right).
\end{align*}
\end{lem}
...
\begin{align*}
  ...
  &= \sum_{a \subset s,t\notin s}^{}\left(\beta \sum_{y \subset a}^{}(x, y) + \sum_{y\subset a \cup \{t\},x \subset a,y\not\subset a}^{}(x, y) \right)\\
  &= \sum_{t\notin s}^{}\left(\sum_{a \subset s}^{}\beta \sum_{y \subset a}^{}(x, y) + \sum_{a \subset s}^{}\sum_{y\subset a \cup \{t\},x \subset a,y\not\subset a}^{}(x, y) \right)\\
  &=\sum_{t\notin s}^{}\left(\beta \binom {|s|-i-1}{n-2i-2}\sum_{y \subset s}^{}(x, y) + \sum_{a \subset s}^{}\sum_{y\subset a \cup \{t\},x \subset a,y\not\subset a}^{}(x, y) \right)\\
  &=\sum_{t\notin s}^{} \left(\beta \binom {|s|-i-1}{n-2i-2}\sum_{y \subset s}^{}(x, y) + \binom {|s|-i}{n-2i-1}\sum_{x \subset s}^{}(x, x \cup \{t\}) \right)
\end{align*}
...
\begin{note}
For any set $s$ with $|s| \geq n-i$, let $u_s = \frac{w_s - (n - |s|)\beta \binom {|s|-i-1}{n-2i-2} z_s}{\binom {|s|-i}{n-2i-1}} + z_s.$
\end{note}

\begin{lem}
\label{u_equivalence}
We have $u_s = \sum_{x\subset s}^{}(x, y).$ In particular, $\sum_{x\subset s}^{}(x, y)$ lies in the span of $v_{(a, b)}.$
\end{lem}
...
\begin{align*}
  ...
  &= \sum_{t\notin s}^{} \sum_{x \subset s}^{}(x, x \cup \{t\}) + \sum_{y \subset s}^{}(x, y) \\
  &=  \sum_{x\subset s}^{}(x, y)
\end{align*}
...
\begin{note}
For any $s \subset [n],$ with $|s| \geq n-i,$ define $h_s = z_{[n]} - u_s.$
\end{note}
...
\begin{note}
For $I$ with $I\subset [n],$ define $l_{I}= \sum_{I \subset x}^{}(x, y).$
\end{note}
\begin{note}
For $I \subset [n]$ let the complement of $I$ be denoted by $I^c = \{i \in [n]\colon i \notin I\}.$
\end{note}
...
Let $I$ be fixed, $i \geq |I| \geq 2$. If $l_J$ lies in the span of $v_{(a, b)}$ for all $J \subset I,$ then so does $l_I.$

\end{lem}
\begin{proof}
Let $I \subset [n].$ Let $A_i = \{x \colon i \in x\}.$ Then, $\cap_{i \in I} A_i = \{x \colon I \subset x\}.$ And, in general, $\cap_{k \in J} A_k = \{x: J \subset x\}$. Using the principle of inclusion exclusion, we can write 
\begin{equation}
\label{pie}\sum_{x \in\cap_{i \in I} A_i} (x, y)= \sum_{x \in \cup_{k \in I} A_k} (x,y) + \sum_{J \subset I, J \neq I, J \neq \emptyset} (-1)^{|I|-|J|+1}\sum_{x \in\cap_{k \in J} A_k} (x, y).
\end{equation}
 Our aim is to show the left hand side of Equation \eqref{pie} lies in the span of $v_{(a,b)}$ so it suffices to show all terms on the right hand of Equation \eqref{pie} side lie in the span of $v_{(a,b)}$. Observe that $\{x \in \cup_{k \in I} A_k\} = \{x \colon x \not \subset I^c\},$ and so $\sum_{x \in \cup_{k \in I} A_k} (x,y) = h_{I^c},$ by Lemma \ref{h_equivalence}. Furthermore, we are assuming $\sum_{x \in\cap_{k \in J} A_k} (x, y),$ lie in the span of $v_{(a, b)},$ for $J \subset I.$ Hence, the right hand side of Equation \eqref{pie} lies in the span of $v_{(a,b)},$ and it follows that $l_I = \sum_{x \in\cap_{i \in I} A_i} (x, y)$ does as well.
...
\end{lem}
\begin{proof}
We prove this by induction on the size of $I.$ First, we check the two base cases: when $|I| = 0, |I| = 1$. In the case $I = \emptyset,$ we know $l_\emptyset$ lies in the span of $v_{(a, b)}$ because $z_{[n]} = l_{\emptyset},$ as $z_{[n]} = \sum_{x \subset y} (x,y) = l_\emptyset,$ from Lemma \ref{z_equivalence}. In the case $|I| = 1$, 
we have $l_I = h_{I^c}$, 
because if $I = \{i\}$, then $\{x \colon x \not\subset I^c\} 
= \{x \colon i \in x\} = \{x \colon I \subset x\}$.

This completes the base case. So, to complete the proof, it suffices to show the induction step: that if we know this Lemma holds for all $|s|>j$ then it holds for $|s| = j.$  This is exactly what Lemma \ref{l_induction_step} proves.
\end{proof}

\begin{lem}
\label{l_diag_equivalence}
For $|x| = i,$ we have $l_{x} = \sum_{y \supset x}^{}(x, y),$ which lies in the span of $v_{(a,b)}$.
\end{lem}
\begin{proof}
This follows immediately from the definition of $l_x$, since the only element of size $i$ containing $x$ is $x$ itself. We know $l_x$ lies in the span of $v_{(a,b)}$ by Lemma \ref{l_in_span}.
\end{proof}
...
The poset $\mathcal E(B_n)$ is unitary Peck for $n > 2$.
\end{cor}
\begin{proof}
