...
\begin{note}
For this section, we will le $M$ be the Lefchetz map on $\mathcal E(B_n)$, as defined in ~\ref{defn:lefschetz}.
\end{note}
\begin{note}
...
In going from the fifth line to the sixth line, for $y,s$ fixed, we count the number of $b$ with $y \subset b \subset s.$ This is exactly $\binom{|s|-i-1}{n-2i-1}$. Then, for each of these posibilities, if we additionally fix $x$, the value of $a$ such that $x \subset a, y \not \subset a$ is uniquely determined by $a = b \setminus (y \setminus x).$
\end{proof}
