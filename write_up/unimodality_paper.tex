...
\end{prop}
\begin{proof}
Since $P$ is self dual, there is an isomorphism $f:P \rightarrow P^{op},$ with inverse $h:P^{op}\rightarrow P.$ Using Lemma \ref{lem:Ff_poset_morphism}, we obtain that $\mathcal E(f)$ and $\mathcal E(g)$ are two sided inverses, and hence $\mathcal E(f):\mathcal E(P) \rightarrow \mathcal E(P^{op})$ is an isomorphism. Furthermore, $\mathcal E(P^{op})$ is canonically isomorphic to $\mathcal E(P)^{op},$ as given by the map $F:\mathcal E(P^{op}) \rightarrow \mathcal E(P)^{op},(x,y) \mapsto (x,y).$ It follows directly from the definitions that $F$ is an isomorphism, as it has the obvious inverse $H: \mathcal E(P)^{op} \rightarrow \mathcal E(P^{op}),(x,y) \mapsto (x,y).$ Then, the composition $\mathcal E(f) \circ H:\mathcal E(P) \rightarrow \mathcal E(P)^{op}$ defines an isomorphism, so $\mathcal E(P)$ is self dual.
\end{proof}

\begin{rem}
\label{rem:induced_action_bn}
Whenever there is an action $\psi:G \times [n] \rightarrow [n],$ we obtain an induced action $\phi: G \times B_n \rightarrow B_n,\phi(g,\{x_1,\ldots, x_k\}) = \{\psi(g,x_1),\ldots, \psi(g,x_k)\}.$ It is well known and easy to see that in fact any action $\phi:G \times B_n \rightarrow B_n$ arises in this way. That is, for any action $\phi$ of $G$ on $B_n$ there exists an action $\psi$ of $G$ on $[n]$ such that $\phi(g,\{x_1,\ldots, x_k\}) = \{\psi(g,x_1),\ldots, \psi(g,x_k)\}.$ Whenever an action $\psi$ of $G$ on $[n]$ is given, we shall refer to the action $\phi$ defined above as the induced action on $B_n.$
\end{rem}
