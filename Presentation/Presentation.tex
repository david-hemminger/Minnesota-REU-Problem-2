...
%%%%%%%ADD BETTER CITATION OF STANLEY'S THEOREM
...
Let $B_n(q)$ be the poset of all $\mathbb F_q$-subspaces of $V_n(q) := (\mathbb F_q)^n$, and $G < Gln(\mathbb F_q)$. We consider $\mathcal E (B_n(q))/G$ and $\mathcal E(B_n(q)/G)$.
\end{block}
\pause
\begin{lem} 
